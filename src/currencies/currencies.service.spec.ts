...
describe('CurrenciesService', () => {
  ...
  describe('getCurrency()', () => {
    ...
    it('should be not throw if repository returns', async () => {
      await expect(service.getCurrency(mockData)).resolves.not.toThrow();
    });

    it('should be called repository with correct params', async () => {
      await service.getCurrency(mockData);
      expect(repository.getCurrency).toBeCalledWith(mockData);
    });

    it('should be return when repository return', async () => {
      (repository.getCurrency as jest.Mock).mockReturnValue(mockData);
      expect(await service.getCurrency(mockData)).toEqual(mockData);
    });
  });
  ...
  describe('createCurrency()', () => {
    ...
    it('should be throw if value <= 0', async () => {
      ...
      expect(service.createCurrency(mockData)).rejects.toThrow(
        new BadRequestException('The value  must be great zero.'),
      );
    });
  });
  ...
  describe('updateCurrency()', () => {
    ...
    it('should be throw if value <= 0', async () => {
      mockData.currency = 0;
      expect(service.updateCurrency(mockData)).rejects.toThrow(
        new BadRequestException('The value  must be great zero.'),
      );
    });
  });
  ...
  describe('deleteCurrency()', () => {
    ...
    it('should be return when repository return', async () => {
      (repository.deleteCurrency as jest.Mock).mockReturnValue(mockData);
      expect(await service.deleteCurrency(mockData)).toEqual(mockData);
    });
